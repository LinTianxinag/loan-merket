...
public class ManPlaServiceImpl implements ManPlaService {
    ...
    public int cancelPla(HttpServletRequest req, ModelAndView model, Platform platform) {
        ...
        if(result == 1){
            result = platformRecordMapper.updateOnshelvesStatus(pr);
        }
        return result;
    }
    ...
    public List<Platform> getPlaList(HttpServletRequest req, ModelAndView model, QueryCondition qc) throws ParseException {
        ...
        List<Platform_Record> plalist = platformRecordMapper.selectPlatformList(qc);
        List<Platform_Record> prlist = platformRecordMapper.selectAll();
        ...
        Integer tdcount = platformRecordMapper.selectTodayCount(startTimes,endTimes);
    ...
    public boolean addPlatForm(HttpServletRequest request, HttpServletResponse response) {
        ...
        //用户选择的位置
        int chooseposit=0;
        if(Integer.parseInt(chooseposition)>=1){
...
//            platformMapper.u(platform);
    ...
    public JSONObject doPlatDown(HttpServletRequest request, HttpServletResponse response) {
        JSONObject object=new JSONObject();
        try{
            String platid=request.getParameter("platid");
            Platform platform=platformMapper.selectByPrimaryKey(Integer.parseInt(platid));
            ...
            platform_record.setOnshelvesStatus("0");
            platform_record.setUpdateTime(new Date());
            platformRecordMapper.updateByPrimaryKey(platform_record);
            object.put("status",200);
            object.put("msg","成功");
    ...
    @Override
    @Transactional(rollbackFor=Exception.class)
    public JSONObject doPlatUpIt(HttpServletRequest request, HttpServletResponse response) {
        JSONObject object=new JSONObject();
        try{
            String platid=request.getParameter("platid");
            Platform platform=platformMapper.selectByPrimaryKey(Integer.parseInt(platid));
            platform.setOnshelves("1");
            platformMapper.updateByPrimaryKey(platform);

            Platform_Record platform_record=platformRecordMapper.getPlatFromByPlatId(Integer.parseInt(platid));
            platform_record.setOnshelvesStatus("1");
            platform_record.setUpdateTime(new Date());
            platformRecordMapper.updateByPrimaryKey(platform_record);
            object.put("status",200);
            object.put("msg","成功");
