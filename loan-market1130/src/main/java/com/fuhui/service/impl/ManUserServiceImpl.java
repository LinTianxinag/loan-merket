...
import com.fuhui.common.util.StringUtils;
...
//        前端paging需要一定格式的返回数据
        ...
        model.addObject("count",page.getTotal());
...
//        前端paging需要一定格式的返回数据
    ...
    public Map getFhSysUserListPager(HttpServletRequest req, QueryCondition qc) throws ParseException {
        Integer pageno = req.getParameter("pageno") != null ? Integer.parseInt(req.getParameter("pageno")) : 1;
        Integer pagesize = req.getParameter("size") != null ? Integer.parseInt(req.getParameter("size")) :10;//        String keyword = req.getParameter("keyword");
        String keyword = req.getParameter("keyword");
        String startTime = req.getParameter("starttime");
        String endTime = req.getParameter("endtime");
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm");
...
//        前端paging需要一定格式的返回数据
     ...
     */
    @Override
    public Object editSysUser(HttpServletRequest req, QueryCondition qc) throws ParseException {
        JSONObject object=new JSONObject();

        try{
            if(req.getParameter("user_id")==null||qc.getUserStatus()==null){
     ...
     */
    @Override
    public List<PlatformBrowsingHistory> getFhUserBrowseDetailsList(HttpServletRequest req, ModelAndView model, QueryCondition qc) throws ParseException {
        Integer pageno = req.getParameter("pageno") != null ? Integer.parseInt(req.getParameter("pageno")) : 1;
    ...
    @Override
    public Map addSysUser(SysUser sysUser) {
        HashMap map = new HashMap();
        if(StringUtils.isEmpty(sysUser.getLoginName())||StringUtils.isEmpty(sysUser.getRealName())||StringUtils.isEmpty(sysUser.getUserPwd())||StringUtils.isEmpty(sysUser.getMobile())||StringUtils.isEmpty(sysUser.getJob())){
            map.put("status",0);
            return map;
        }
        sysUser.setUserStatus(Byte.valueOf("1"));
        sysUser.setUserPwd(MD5.GetMD5Code(sysUser.getUserPwd()));
